...
class McpService {
    ...
    async download_file(url: string, saveFile: string) {
        ...
        // 发起下载请求
        let headers = {
            'User-Agent': 'AiToDesk/' + pub.version()
        };
        let downloadBytes = 0;
    ...
    async install_npx() {
        ...
        let downloadUrl = `https://aitodesk.bt.cn/bin/${os_path}/bun.zip`
    ...
    async sync_cloud_mcp(){
        let downloadUrl = `https://aitodesk.bt.cn/config/common-mcp-server.json`;
